/* =========================================================
   ...
   Persistência em localStorage + Estatísticas do Turno
   Tema: Dark Elegant
   ========================================================= */
...
// garante estrutura de turno
if (!state.shift) {
  state.shift = { totalSales: 0, closedCount: 0, startedAt: Date.now() };
}
...
// Map de fundos decorativos por categoria (coloque os SVG/PNGs em /img/)
const CAT_BG = {
  cervejas:   "img/beer-bg.svg",
  naoalcool:  "img/soda-bg.svg",
  petiscos:   "img/petisco-bg.svg",
  espetinhos: "img/skewer-bg.svg",
  sanduiches: "img/burger-bg.svg",
  drinks:     "img/drinks-bg.svg",
};
...
const PRODUCTS = {
  cervejas: [
    ...
    {id:'cer3', name:'Heineken 600ml', price:16.9},
...
function createInitialState(){
  ...
  return { tables, currentUser: null, currentMesa: null, currentCategory: 'cervejas', shift:null };
}
function loadState(){
  try{ const raw = localStorage.getItem(APP_KEY); return raw ? JSON.parse(raw) : null; }
  catch(e){ return null; }
}
    ...
    col.innerHTML = `
      <div class="mesa-card" data-mesa="${i}">
        <div class="d-flex justify-content-between align-items-start mb-2">
          <div class="mesa-title"><i class="bi bi-table"></i> Mesa ${i}</div>
          <span class="badge-status ${ocupada?'badge-ocupada':'badge-livre'}">
            ${ocupada ? 'Ocupada' : 'Livre'}
          </span>
        </div>
        <div class="small text-muted">Total até o momento</div>
        <div class="fs-5 fw-bold">${fmtBRL(Math.max(total,0))}</div>
      </div>
    `;
    col.querySelector('.mesa-card').addEventListener('click',()=>{ openMesa(i); });
...
function renderMesa(){
  ...
  renderProducts();
  renderCart();
  setUserGreeting();  // saudação também aqui (versão reduzida)
  renderTopStats();   // atualiza stats da faixa
}
...
function renderProducts(){
  ...
  const bg = CAT_BG[state.currentCategory] || '';

  list.forEach(p=>{
    const col = document.createElement('div');
    col.className = 'col-6 col-md-4';
    col.innerHTML = `
      <div class="prod-card" style="${bg ? `
...
function linkAdjustInputs(){
  ...
  getEl('btnFecharComanda').onclick = ()=>{
    const total = Math.max(getTotals().total, 0);
    if(confirm(`Fechar comanda da Mesa ${state.currentMesa} no valor de ${fmtBRL(total)}?`)){
      // Acumula no turno
      if (!state.shift) state.shift = { totalSales: 0, closedCount: 0, startedAt: Date.now() };
      state.shift.totalSales = (state.shift.totalSales || 0) + total;
      state.shift.closedCount = (state.shift.closedCount || 0) + 1;
...
function getOpenTablesCount(){
  ...
  for (let i=1;i<=10;i++){
    ...
    const ajustes = calcAdjustments(mesa, subtotal);
    const total = subtotal + ajustes;
    if (mesa.items.length > 0 || total > 0) open++;
  }
  return open;
}
function renderTopStats(){
  const elA = document.getElementById('statMesasAbertas');
  const elV = document.getElementById('statVendasTurno');
  if (elA) elA.textContent = getOpenTablesCount();
  if (elV) elV.textContent = fmtBRL(state.shift?.totalSales || 0);

  const elAM = document.getElementById('statMesasAbertasMesa');
  const elVM = document.getElementById('statVendasTurnoMesa');
  if (elAM) elAM.textContent = getOpenTablesCount();
  if (elVM) elVM.textContent = fmtBRL(state.shift?.totalSales || 0);
}

document.addEventListener('DOMContentLoaded', ()=>{
  const btnZ = document.getElementById('btnZerarTurno');
  if (btnZ){
    btnZ.addEventListener('click', ()=>{
      if (confirm('Zerar estatísticas do turno (vendas e contadores)?')){
        state.shift = { totalSales: 0, closedCount: 0, startedAt: Date.now() };
        saveState();
        renderTopStats();
        renderDashboard();
...
function setUserGreeting(){
  ...
  const el2 = document.getElementById('userNameMesa');
  if (el1) el1.textContent = state.currentUser || 'Usuário';
  if (el2) el2.textContent = state.currentUser || 'Usuário';
}
